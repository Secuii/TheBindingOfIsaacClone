...
public class LevelGeneration : MonoBehaviour
{
    ...
    {
        foreach (Transform rooms in mapRoot)
        {
	...
	void CreateRooms()
	{
		...
		roomsObjects = new GameObject[gridSizeX * 2, gridSizeY * 2];
	...
	void DrawMap()
	{
		foreach (Room room in rooms)
		{
			...
			drawPos.x *= 17.78f;//aspect ratio of map sprite
			drawPos.y *= 9.98f;
			//create map obj and assign its variables
			MapRoomSelector mapper = Object.Instantiate(roomWhiteObj, drawPos, Quaternion.identity, mapRoot).GetComponent<MapRoomSelector>();
			mapper.name = "Room";
			...
			for (int i = 0; i < worldSize.x * 2; i++)
			{
				for (int j = 0; j < worldSize.y * 2; j++)
				{
					if(room == rooms[i, j])
                    {
						roomsObjects[i, j] = mapper.gameObject;
                    }
				}
			}
			StartCoroutine(CheclChildRoom(room, rooms, roomsObjects));
		}
	}

	IEnumerator CheclChildRoom(Room room, Room[,] rooms, GameObject[,] mapper)
	{
		...
		for (int i = 0; i < worldSize.x * 2; i++)
		{
			for (int j = 0; j < worldSize.y * 2; j++)
			{
				if (room == rooms[i, j])
				{
					if (room.doorTop)
					{
						mapper[i, j].transform.GetChild(0).GetComponent<RoomController>().UpRoom = mapper[i, j + 1].transform.GetChild(0).gameObject;
					}
					if (room.doorBot)
					{
						mapper[i, j].transform.GetChild(0).GetComponent<RoomController>().DownRoom = mapper[i, j - 1].transform.GetChild(0).gameObject;
					}
					if (room.doorLeft)
					{
						mapper[i, j].transform.GetChild(0).GetComponent<RoomController>().LeftRoom = mapper[i - 1, j].transform.GetChild(0).gameObject;
					}
					if (room.doorRight)
					{
						mapper[i, j].transform.GetChild(0).GetComponent<RoomController>().RightRoom = mapper[i + 1, j].transform.GetChild(0).gameObject;
					}
				}
			}
		}
